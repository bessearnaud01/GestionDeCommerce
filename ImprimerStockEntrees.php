...
class MYPDF extends TCPDF {
    ...
    public function Header(){
        ...
        $this->Cell(0,10,'La listes des stoks entrant',0,false,'C');
 
     }


    // Colored table
    public function ColoredTable($header,$StockesEntrees) {
        ...
        // Data
        $fill = 0;
        foreach($StockesEntrees as $row) {
            $this->Cell($w[0],6, $row-> getIdStocke(), 'LR', 0, 'L', $fill);
            $this->Cell($w[1],6, $row-> getIdFournisseur(), 'LR', 0, 'L', $fill);
...
// print colored table
$pdf->ColoredTable($header, $StockesEntrees);
