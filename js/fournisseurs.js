...
let maxpages = Math.ceil(fournisseurs.length / numberOfItems);
let tableList = '';
...
function showList(){
    tableList=''  
        for(let i = first; i < first + numberOfItems;i++){
         if(i<fournisseurs.length){
          tableList += `
          <tr>
          <td> ${fournisseurs[i].idFournisseur}</td>
          <td> ${fournisseurs[i].idProduit}</td>
          <td> ${fournisseurs[i].entreprise}</td>
          <td> ${fournisseurs[i].telephone} </td>
          <td> ${fournisseurs[i].email} </td>
          <td> ${fournisseurs[i].adresse} </td>
          <td> <a href="EditeFournisseur.php?&id=${fournisseurs[i].idFournisseur}" target="_self"><i class="fas fa-edit"></i></a> &nbsp
          <a onclick ="return confirm(' Êtes vous sûr de vouloir supprimer le fournisseur')";
           href="SupprimeFournisseur.php?&id=${fournisseurs[i].idFournisseur}" target="_self"> <i class="fas fa-trash-alt"></i> </a>
          </td>
      </tr>
    ...
    document.getElementById('fournisseurs').innerHTML = tableList;
    showPageInfo();
  }
