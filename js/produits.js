...
let maxpages = Math.ceil(produits.length / numberOfItems);
let tableList = '';
...
function showList() {
    ...
    for (let i = first; i < first + numberOfItems; i++) {
        if (i < produits.length) {
            tableList += `
          <tr>
          <td> ${produits[i].idProduit} </td>
          <td> ${produits[i].nomProduit} </td>
          <td> ${produits[i].marque}</td>
          <td> ${produits[i].pa}</td>
          <td><img src="photos/${produits[i].image}" width="70" height="70" /></td>
          <td> <a href="EditerProduit.php?&id=${produits[i].idProduit}}" target="_self"><i class="fas fa-edit"></i></a> &nbsp
          <a onclick ="return confirm('Êtes vous sûr de vouloir supprimer le produit')";
           href="SupprimeProduit.php?&id=${produits[i].idProduit}" target="_self"> <i class="fas fa-trash-alt"></i> </a>
          </td>
      </tr>
    ...
    document.getElementById('produits').innerHTML = tableList;
    showPageInfo();
}
