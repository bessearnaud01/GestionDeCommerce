...
let maxpages = Math.ceil(familles.length / numberOfItems);
let tableList = '';
...
function showList(){
    tableList=''  
        for(let i = first; i < first + numberOfItems;i++){
         if(i<familles.length){
          tableList += `
          <tr>
            <td>${familles[i].idFamille}</td>
            <td>${familles[i].libelle}</td>
            <td> <a href="EditerFamille.php?&id=${familles[i].idFamille}" target="_self"><i class="fas fa-edit"></i></a> &nbsp
            <a onclick ="return confirm(' Êtes vous sûr de vouloir supprimer la famille du produit')";
            href="SupprimerFamille.php?&id=${familles[i].idFamille}" target="_self"><i class="fas fa-trash-alt"></i> </a>
            </td>
          </tr>
    ...
    document.getElementById('familles').innerHTML = tableList;
    showPageInfo();
  }
