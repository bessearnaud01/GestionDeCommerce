...
let maxpages = Math.ceil(factures.length / numberOfItems);
let tableList = '';
...
function showList() {
    ...
    for (let i = first; i < first + numberOfItems; i++) {
        if (i < factures.length) {
            tableList += `
        <tr>
            <td> ${factures[i].idFacture} </td>
            <td> ${factures[i].nomClient} </td>
            <td> ${factures[i].nomProduit} </td>
            <td> ${factures[i].libelle} </td>
            <td> ${factures[i].prixUnitaire} </td>
            <td> ${factures[i].quantite}</td>
            <td> ${factures[i].montant} </td>
            <td> ${factures[i].date} </td>
            <td> <a href="EditeFacture.php?&id=${factures[i].idFacture}" target="_self"><i class="fas fa-edit "></i></a> 
            <a onclick ="return confirm(' Êtes vous sûr de vouloir supprimer la famille du produit')";
            href="SupprimeFacture.php?&id=${factures[i].idFacture}" target="_self"><i class="fas fa-trash-alt "></i> </a>
            <a href="ImprimeFacture.php?&id=${factures[i].idFacture}" target="_self"><i class="fas fa-print "></i></a> 
            </td>
       </tr>
    ...
    document.getElementById('factures').innerHTML = tableList;
    showPageInfo();
}
